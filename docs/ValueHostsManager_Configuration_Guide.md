...
The `ValueHostsManager` is the central object you use with a form or model. It contains the list of fields (called `ValueHosts`), their rules, and more.
...
{
  // see ValueHostsManagerConfigBuilder class for field(), static(), calc(), whenToEnable(), and state
...
- `field()`, `static()`, `calc()`, `whenToEnable()`, several others are inherited from the Builder. See [`ValueHostsManagerConfigBuilder class`](#the-valuehostsmanagerconfigbuilder-class).
