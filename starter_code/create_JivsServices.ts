 ...
 * 
 * It contains the createJivsServices() function which you will use to create and configure the JivsServices object.
 ...
 * - ErrorMessagesService: You must provide text for error messages. This supplies both default and localized text.
 * 
...
import
    {
        CaseInsensitiveStringConverter, DateTimeConverter,
        IntegerConverter,
        LocalDateOnlyConverter,
        NumericStringToNumberConverter,
        TimeOfDayHMSOnlyConverter,
        TimeOfDayOnlyConverter,
        UTCDateOnlyConverter
    } from "@plblum/jivs-engine/build/DataTypes/DataTypeConverters";
import
    {
        AbbrevDateFormatter,
        AbbrevDOWDateFormatter,
        BooleanFormatter,
        CapitalizeStringFormatter,
        CurrencyFormatter,
        DateFormatter,
        DateTimeFormatter,
        IntegerFormatter,
        LongDateFormatter, LongDOWDateFormatter,
        LowercaseStringFormatter,
        NumberFormatter,
        Percentage100Formatter,
        PercentageFormatter,
        StringFormatter,
...
import { IErrorMessagesService } from "@plblum/jivs-engine/build/Interfaces/ErrorMessagesService";
...
import { HtmlMessageTokenResolverService } from '@plblum/jivs-engine/build/Services/HtmlMessageTokenResolverService';
import { ErrorMessagesService } from "@plblum/jivs-engine/build/Services/ErrorMessagesService";
import { registerStandardValueHostGenerators, ValueHostFactory } from '@plblum/jivs-engine/build/ValueHosts/ValueHostFactory';
 ...
 * JivsServices provides services and factories for dependency injection throughout the application.
 * It is intended to let you customize most of the features within your Jivs instance.
 * 
 * ## Services that you often have to setup
 * - `CultureService` - Identifies the ISO language-region names that your app supports.
 * - `ErrorMessagesService` - Supports localized strings for error messages, labels, and more. 
 * - `LoggingService` - Provides logging.
 * - `LookupKeyFallbackService` - When you create a new Lookup Key, update this to give it a fallback Lookup Key 
 ...
 * - `DataTypeFormatterService`: Register your own `DataTypeFormatters`.
 ...
 * 
 * When you write unit tests, you will use this too, but only if testing runtime app code.
 * If you are writing new classes for Jivs, like a parser or condition, consider
 * calling createJivsServicesForTesting() found in the Support/createJivsServicesForTesting.ts file.
 * It gives you a light starting place with options to bring in the services and factories you need for testing.
 * 
...
*/
     ...
     */
    function registerCultures(cs: ICultureService): void
    {
    ...
    function createErrorMessagesService(usage: 'client' | 'server' | 'all' = 'client'): IErrorMessagesService
    {
        let service = new ErrorMessagesService();
        service.lazyLoad = (tls) =>
        {
            ...
            // create a lookup key using this pattern, and see if the ErrorMessagesService has a value for it.
    ...
    function createConditionFactory(): ConditionFactory
    {
        let cf = new ConditionFactory();
        registerConditions(cf);
        return cf;
    }
    function registerConditions(cf: ConditionFactory): void
    {
     ...
     * @param dtis 
     */
    function registerDataTypeIdentifiers(dtis: DataTypeIdentifierService): void
    {
     ...
     * @param dtfs 
     */
    function registerDataTypeFormatters(dtfs: DataTypeFormatterService): void
    {
        ...
        dtfs.lazyLoad = (service) =>
        {
            ...
            // less used - consider commenting out until you know they are needed
            ...
            // NOTE: BooleanFormatter has its strings localized in JivsServices.ErrorMessagesService
     ...
     * @param dtcs 
     */
    function registerDataTypeConverters(dtcs: DataTypeConverterService): void
    {
        // NOTE: Lazy loads here, but its optional, and you can register some before attaching the callback
        // for cases that you know will be needed early on.
     ...
     * @param dtcs 
     */
    function registerDataTypeComparers(dtcs: DataTypeComparerService): void
    {
        /* These are pre-installed into DataTypeIdentifierService as they are core functionality
    ...
    /**
     * Give data types their parsers, for use with FieldValueHosts.
     ...
     * @param dtps 
     */
    function registerDataTypeParsers(dtps: DataTypeParserService): void
    {
        dtps.enabled = true;
        // NOTE: Lazy loads here, but its optional, and you can register some before attaching the callback
        // for cases that you know will be needed early on.
...
*/            
     ...
     * @returns 
     */
    function createLookupKeyFallbackService(): ILookupKeyFallbackService
    {
    ...
    function createConfigMergeServices(vs: JivsServices): void
    {
