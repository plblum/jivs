...
import { DataTypeCheckCondition, RequiredTextCondition, RegExpCondition, RangeCondition, EqualToCondition, NotEqualToCondition, GreaterThanCondition, LessThanCondition, GreaterThanOrEqualToCondition, LessThanOrEqualToCondition, StringLengthCondition, AllMatchCondition, AnyMatchCondition, CountMatchesCondition, StringNotEmptyCondition, NotNullCondition } from "../src/Conditions/ConcreteConditions";
import { ConditionFactory } from "../src/Conditions/ConditionFactory";
import { ConditionType } from "../src/Conditions/ConditionTypes";
import { CaseInsensitiveStringConverter, DateTimeConverter, LocalDateOnlyConverter, TotalDaysConverter, IntegerConverter, TimeOfDayOnlyConverter, TimeOfDayHMSOnlyConverter } from "../src/DataTypes/DataTypeConverters";
import { StringFormatter, NumberFormatter, IntegerFormatter, DateFormatter, CapitalizeStringFormatter, UppercaseStringFormatter, LowercaseStringFormatter, DateTimeFormatter, AbbrevDateFormatter, AbbrevDOWDateFormatter, LongDateFormatter, LongDOWDateFormatter, TimeofDayFormatter, TimeofDayHMSFormatter, BooleanFormatter, CurrencyFormatter, PercentageFormatter, Percentage100Formatter } from "../src/DataTypes/DataTypeFormatters";
...
export function registerConditions(cf) {
    // Register the desired conditions, including adding your own
    cf.register(ConditionType.DataTypeCheck, (descriptor) => new DataTypeCheckCondition(descriptor));
    cf.register(ConditionType.RequiredText, (descriptor) => new RequiredTextCondition(descriptor));
    cf.register(ConditionType.RegExp, (descriptor) => new RegExpCondition(descriptor));
    cf.register(ConditionType.Range, (descriptor) => new RangeCondition(descriptor));
    cf.register(ConditionType.EqualTo, (descriptor) => new EqualToCondition(descriptor));
    cf.register(ConditionType.NotEqualTo, (descriptor) => new NotEqualToCondition(descriptor));
    cf.register(ConditionType.GreaterThan, (descriptor) => new GreaterThanCondition(descriptor));
    cf.register(ConditionType.LessThan, (descriptor) => new LessThanCondition(descriptor));
    cf.register(ConditionType.GreaterThanOrEqualTo, (descriptor) => new GreaterThanOrEqualToCondition(descriptor));
    cf.register(ConditionType.LessThanOrEqualTo, (descriptor) => new LessThanOrEqualToCondition(descriptor));
    cf.register(ConditionType.StringLength, (descriptor) => new StringLengthCondition(descriptor));
    cf.register(ConditionType.And, (descriptor) => new AllMatchCondition(descriptor));
    cf.register(ConditionType.Or, (descriptor) => new AnyMatchCondition(descriptor));
    cf.register(ConditionType.CountMatches, (descriptor) => new CountMatchesCondition(descriptor));
    // StringNotEmpty is similar to RequiredText, but lacks evaluating as the user types
    cf.register(ConditionType.StringNotEmpty, (descriptor) => new StringNotEmptyCondition(descriptor));
    cf.register(ConditionType.NotNull, (descriptor) => new NotNullCondition(descriptor));
    // aliases for users who don't deal well with boolean logic can relate
    cf.register(ConditionType.All, (descriptor) => new AllMatchCondition(descriptor));
    cf.register(ConditionType.Any, (descriptor) => new AnyMatchCondition(descriptor));
}
/**
 * Cultures that you want to localize.
 * -> Create an array of CultureIdFallback objects in configureCultures()
 */
export function configureCultures() {
    return [
 ...
 * @param dtis
 */
export function registerDataTypeIdentifiers(dtis) {
 ...
 * @param dtfs
 */
export function registerDataTypeFormatters(dtfs) {
 ...
 * @param dtcs
 */
export function registerDataTypeConverters(dtcs) {
 ...
 * @param dtcs
 */
export function registerDataTypeComparers(dtcs) {
}
/**
 ...
 * @param ag
 */
export function registerDataTypeCheckGenerators(ag) {
    // See \examples\EmailAddressDataType.ts for example.
    //    ag.register(new EmailAddressDataTypeCheckConverter());
}
export function createTextLocalizerService() {
...
//# sourceMappingURL=create_services.js.map