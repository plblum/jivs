...
// 5. Register new classes with the JivsServices object in createJivsServices.
//    - Condition class in the JivsServices.conditionFactory
//    - IDataTypeCheckGenerator subclass in JivsServices.dataTypeCheckGeneratorService
...
export function registerEmailAddress(services: IJivsServices): void
{
    ...
    ff.setValidatorBuilderCreator((parentConfig: FieldValueHostConfig) => {
        return new EmailAddressValidatorBuilder(services, parentConfig) as unknown as IValidatorBuilder;
    });
    ff.setConditionBuilderCreator(
        (parentBuilder: IBuilderConfigHost<object>, completed?: CompleteConfigBuilderHandler<any>) => {
        return new EmailAddressConditionBuilder(services, parentBuilder, completed) as unknown as IConditionBuilder;
    });    
}
