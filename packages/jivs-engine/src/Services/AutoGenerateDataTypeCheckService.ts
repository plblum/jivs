    ...
    implements IAutoGenerateDataTypeCheckService
{
    ...
    public autoGenerateDataTypeCondition(valueHost: IInputValueHost, dataTypeLookupKey: string): ICondition | null
    {
        ...
        try {
            ...
            if (generator !== null) {
                this.log(() => `Using ${valueForLog(generator)}`, LoggingLevel.Debug);
                result = generator.createCondition(valueHost, dataTypeLookupKey, this.services.conditionFactory); // may return null
                return result;
            }
            let config = this.createDefaultConditionConfig(valueHost);
            result = this.services.conditionFactory.create(config);
            return result;
        }
        finally
        {
            this.log(() => `Auto generated: ${valueForLog(result)}`, LoggingLevel.Info);
        }
    }
