 ...
 */
    ...
    protected tryAutoGenerateDataTypeCheckCondition(validators: Array<IValidator>): boolean {
        let created = false;
        if (this.services.dataTypeCheckGeneratorService.enabled) {
            const lookupKey = this.getDataType();
            if (lookupKey) {
                const dtcConditions = this.services.dataTypeCheckGeneratorService.autoGenerateDataTypeConditions(this, lookupKey);
    ...
    {
        // will write to the FieldValueHostState so that it is persisted across state updates.
        this.updateInstanceState((stateToUpdate) => {
