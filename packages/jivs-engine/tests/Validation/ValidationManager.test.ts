...
import {
    IValidationManager,
    IValidationManagerAccessor,
    IValidationManagerCallbacks, ValidationManagerConfig, ValidationManagerInstanceState, ValidationManagerInstanceStateChangedHandler, toIValidationManager,
    toIValidationManagerAccessor,
...
describe('constructor and initial property values', () => {
    ...
    test('Callbacks supplied. Other parameters are null', () => {
        let setup: ValidationManagerConfig = {
            ...
            onConfigChanged: (manager: IValidationManager, valueHostConfigs: Array<ValueHostConfig>) => { }
        };
...
function setupValidationManager(configs?: Array<FieldValueHostConfig> | null,
...
function testStaticValueHostInstanceState(testItem: PublicifiedValidationManager, valueHostName: ValueHostName,
...
describe('ValidationManager.addValueHost', () => {
    ...
    test('Second ValueHost with same name throws', () => {
        let testItem = new PublicifiedValidationManager({ services: new MockJivsServices(false, false), valueHostConfigs: [] });
...
describe('ValidationManager.addOrMergeValueHost', () => {
    test('Replace the config to install a validator', () => {
        let testItem = new PublicifiedValidationManager({ services: new MockJivsServices(false, false), valueHostConfigs: [] });
        let config: FieldValueHostConfig = {
            name: 'Field1',
...
describe('validate, and isValid, doNotSave, getIssuesForField, getIssuesFound based on the results', () => {
    test('Before calling validate with 0 fieldValueHosts, isValid=true, doNotSave=false, getIssuesForField=[], getIssuesFound=[]', () => {
        ...
        expect(setup.validationManager.getIssuesForField('Anything')).toBeNull();
    ...
    test('isValid is true and doNotSave is false before calling validate with 1 fieldValueHosts', () => {
        ...
        expect(setup.validationManager.isValid).toBe(true);
        expect(setup.validationManager.doNotSave).toBe(false);
        expect(setup.validationManager.getIssuesForField(config.name)).toBeNull();
        expect(setup.validationManager.getIssuesFound()).toBeNull();
    });
    ...
    test('With 1 fieldValueHost that is ValidationStatus.Invalid, returns {isValid:false, doNotSave: true, issuesFound: [1 found] }', () => {
        ...
        expect(setup.validationManager.getIssuesForField(config.name)).toEqual([expectedIssueFound]);
    ...
    test('With 2 fieldValueHost that are both Invalid, returns {isValid:false, doNotSave: true, issuesFound: [2 entries]}', () => {
        ...
        expect(setup.validationManager.getIssuesForField(config1.name)).toEqual([expectedIssueFound]);
        expect(setup.validationManager.getIssuesForField(config2.name)).toEqual([expectedIssueFound2]);
    ...
    test('With 1 external IssueFound not associated with any ValueHost, isValid=false, DoNotSave=true, getIssuesFound has the external IssueFound, and there is a new ValueHost for the BusinessLogic', () => {
        ...
        expect(setup.validationManager.getIssuesForField(ModelValidatorsValueHostName)).toEqual([<IssueFound>{
            errorMessage: 'BL_ERROR',
            severity: ValidationSeverity.Error,
            valueHostName: ModelValidatorsValueHostName,
            errorCode: 'GENERATED_0',
    ...
    test('With 1 ValueHost that is assigned without validators 1 external IssueFound, isValid=false, DoNotSave=true, getIssuesFound has the external IssueFound, and there is a no ValueHost for the BusinessLogic', () => {
        ...
        expect(setup.validationManager.getIssuesForField(config.name)).toEqual([<IssueFound>{
            errorMessage: 'BL_ERROR',
            severity: ValidationSeverity.Error,
            valueHostName: config.name,
            errorCode: 'GENERATED_0',
        ...
        expect(setup.validationManager.getIssuesForField(ModelValidatorsValueHostName)).toBeNull();
    });
    test('With 1 ValueHost that is assigned with 1 validator that is NoMatch, 1 external IssueFound not associated with a ValueHost, isValid=false, DoNotSave=true, getIssuesFound has both errors external IssueFound, BLValueHost has the BLError, FieldValueHost has its own error', () => {
        ...
        expect(setup.validationManager.getValueHost(ModelValidatorsValueHostName)).toBeInstanceOf(ModelValidatorsValueHost);
        expect(setup.validationManager.getIssuesForField(ModelValidatorsValueHostName)).toEqual(
            [<IssueFound>{
                errorMessage: 'BL_ERROR',
                severity: ValidationSeverity.Error,
                valueHostName: ModelValidatorsValueHostName,
    ...
    test('With 1 fieldValueHost and a condition that will evaluate as NoMatch, use option Preliminary=true, expect ValidationStatus.Undetermined because Require should be skipped, leaving NO validators', () => {
        ...
        (setup.validationManager.getValueHost('Field1')! as IFieldValueHost).setTextValue('');
    ...
    test('With 1 fieldValueHost and a condition that will evaluate as NoMatch, use option Preliminary=false, expect ValidationStatus.Invalid because Preliminary is off', () => {
        ...
        (setup.validationManager.getValueHost('Field1')! as IFieldValueHost).setValue('');
        let validationState: ValidationState | null = null;
    ...
    test('With 1 fieldValueHost and a condition that will evaluate as NoMatch during edit, use option DuringEdit=true, expect Invalid', () => {
        ...
        setup.validationManager.getFieldValueHost('Field1')?.setTextValue(''); // requires text for duringEdit
        let validationState: ValidationState | null = null;
        expect(() => validationState = setup.validationManager.validate({ duringEdit: true })).not.toThrow();
        expect(validationState).toEqual(<ValidationState>{
            isValid: false,
    ...
    test('With 1 fieldValueHost that has a string value and a condition that will evaluate as Match during edit, use option DuringEdit=true, expect Valid', () => {
        let config = setupFieldValueHostConfig(0, [AlwaysMatchesConditionType]);
        let setup = setupValidationManager([config]);
        setup.validationManager.getFieldValueHost(config.name)?.setTextValue('Text');
        let validationState: ValidationState | null = null;
        expect(() => validationState = setup.validationManager.validate({ duringEdit: true })).not.toThrow();
        expect(validationState).toEqual(<ValidationState>{
            isValid: true,
    ...
    test('With 1 fieldValueHost that has an undefined value and a condition that will evaluate as Match during edit, use option DuringEdit=true, expect Undetermined because DuringEdit requires a string value', () => {
        ...
        setup.validationManager.getFieldValueHost(config.name)?.setTextValue(undefined);
        let validationState: ValidationState | null = null;
        expect(() => validationState = setup.validationManager.validate({ duringEdit: true })).not.toThrow();
        expect(validationState).toEqual(<ValidationState>{
            isValid: true,
    ...
    test('With 1 fieldValueHost and a condition that does not implement IEvaluateConditionDuringEdits, use option DuringEdit=true, expect condition to be skipped and array of ValidateResults = []', () => {
        ...
        let vh = (setup.validationManager.getValueHost('Field1')! as IFieldValueHost);
        vh.setTextValue('');
    ...
    test('When ValueHost.isEnabled=false, even though validator is valid, result is as if the validator did not get used because its own ValidationStatus=Disabled', () => {
        let config = setupFieldValueHostConfig(0, [AlwaysMatchesConditionType]);
        let setup = setupValidationManager([config]);
        let vh = setup.validationManager.getFieldValueHost(config.name)!;
        vh.setEnabled(false);
        setup.validationManager.validate();
    ...
    test('validate() passes final ValidationState to callback', () => {
        ...
        let testItem = new ValidationManager(vmConfig);
...
describe('invokeOnConfigChanged', () => {
    function testCallback(configs: Array<ValueHostConfig>): void {
        function handler(manager: IValidationManager, configs: Array<ValueHostConfig>): void {
...
describe('dispose', () => {
    ...
    test('dispose with valueHostsConfig having its own dispose kills what the config.dispose expects', () => {
        interface X extends ValidationManagerConfig, IDisposable {
            x: {}
        }
        let vmConfig: X = {
            services: new MockJivsServices(true, false), valueHostConfigs: [],
...
describe('toIValueHostResolver function', () => {
    ...
    test('ValidationManager matches and returns itself.', () => {
        let testItem = new ValidationManager<ValidationManagerInstanceState>({
            services: new JivsServices(),
            valueHostConfigs: []
        });
        expect(toIValueHostResolver(testItem)).toBe(testItem);
...
describe('toIValidationManagerAccessor function', () => {
    test('Matches interface returns strongly typed object.', () => {
        let testItem: IValidationManagerAccessor = {
            validationManager: {
                getValueHost: (name) => { return <any>{}; },
                vh: {} as unknown as IValueHostAccessor,
                getCalcValueHost: (name) => { return <any>{}; },
        ...
        expect(toIValidationManagerAccessor(testItem)).toBe(testItem);
    });
    ...
    test('Non-matching interface returns null.', () => {
        let testItem = {};
        expect(toIValidationManagerAccessor(testItem)).toBeNull();
    });
    test('null returns null.', () => {
        expect(toIValidationManagerAccessor(null)).toBeNull();
    });
    test('Non-object returns null.', () => {
        expect(toIValidationManagerAccessor(100)).toBeNull();
    });
});
describe('Round trip caching of Config and State', () => {
    test('With capturing of state and config setup, create another ValidationManager using the captured data to confirm it rebuilds the same config', () => {
        function captureConfig(ValidationManager: IValidationManager, valueHostConfigs: Array<ValueHostConfig>): void {
            capturedConfig = valueHostConfigs;
        }
        let capturedConfig: Array<ValueHostConfig> | undefined = undefined;
        function captureManagerStateChanged(ValidationManager: IValidationManager, stateToRetain: ValidationManagerInstanceState): void {
            capturedManagerState = stateToRetain;
        }
        let capturedManagerState: ValidationManagerInstanceState | undefined = undefined;
...
describe('fromValidationPayload()', () => {
    ...
    test('preserves existing validator-managed issues while importing external issues', () => {
        ...
        let target = new ValidationManager(vmConfig);
        target.getFieldValueHost('Field1')!.setValues('', '');
        target.validate();

        let source = new ValidationManager(vmConfig);
    ...
    test('imports a server issue for a field missing on the client by routing it to ModelValidatorsValueHost', () => {
        ...
        let serverConfig = <ValidationManagerConfig>{ services: serverServices, valueHostConfigs: [] };
        ...
        let clientConfig = <ValidationManagerConfig>{ services: clientServices, valueHostConfigs: [] };
