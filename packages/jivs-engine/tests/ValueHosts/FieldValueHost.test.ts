...
describe('constructor and resulting property values', () => {
    ...
    test('constructor with valid parameters created and sets up Services, Config, and InstanceState', () => {
        ...
        expect(testItem!.getElementIdentifier()).toBeUndefined();
    });
...
{
    test('getElementIdentifier defaults to undefined', () =>
    {
        let setup = setupFieldValueHost();
        expect(setup.valueHost.getElementIdentifier()).toBeUndefined();
    });
    // pass template with {0} but no elementIdentifier set. Expect getElementIdentifier to return undefined
    test('getElementIdentifier returns undefined when template has {0} but no elementIdentifier set', () =>
    {
        let setup = setupFieldValueHost({
            name: 'Field1'
        });
        expect(setup.valueHost.getElementIdentifier("{0}")).toBeUndefined();
    });
