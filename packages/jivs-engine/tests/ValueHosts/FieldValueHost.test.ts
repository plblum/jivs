...
import { IntegerDataTypeCheckGenerator, RegExpDataTypeCheckGenerator } from '../../src/DataTypes/DataTypeCheckGenerators';
...
function setupFieldValueHost(
    ...
    (services.loggingService as TestingLoggingService).chainedLogger = new ConsoleLoggingService(services.loggingService.minLevel);
    (services.loggingService as TestingLoggingService).overrideMinLevelWhen({ hasData: true });
...
describe('setTextValue with parserLookupKey enabled to see both text value and native values are assigned', () => {
        ...
        ): ITestSetupConfig
    {
        ...
        let setup = setupFieldValueHost(ivh);
        setup.services.loggingService.minLevel = LoggingLevel.Debug;
 //       (setup.services.loggingService as TestingLoggingService).
        registerDataTypeParsers(setup.services.dataTypeParserService);
        setup.services.lookupKeyFallbackService.register(LookupKey.Integer, LookupKey.Number);
...
describe('FieldValueHost.setValues with getTextValue and getValue to check result', () => {
    test('Text value of "10", Value of 10, options is undefined. Sets both values, IsChanged = true, and does not validate', () => {
        ...
        expect(valueHostState.changeCounter).toBe(1);
    ...
    test('Text value of "10", Value of 10, options is { validate: true }. Sets both values, IsChanged = true, and validate (no Validators to cause Invalid, so result is Undetermined)', () => {
        ...
        expect(valueHostState.value).toBe(10);
        expect(valueHostState.textValue).toBe("10");
        expect(valueHostState.status).toBe(ValidationStatus.Undetermined);
        expect(valueHostState.changeCounter).toBe(1);
    });
...
describe('FieldValueHost.validate uses autogenerated DataTypeCheck condition', () => {
    test('No conditions at all. DataTypeCheckCondition gets added', () => {
        ...
        setup.services.dataTypeCheckGeneratorService.enabled = true;
        ...
        let ivConfigs: Array<Partial<ValidatorConfig>> = [
            {
                ...
                errorMessage: null, // so we can use ErrorMessagesService
                summaryMessage: null
            }
        ];
        let state: Partial<FieldValueHostInstanceState> = {};
    ...
    test('1 condition and it has ConditionCategory=DataTypeCheck. No DataTypeCheckCondition gets added.', () => {
        ...
        logger.minLevel = LoggingLevel.Info;
        setup.services.dataTypeCheckGeneratorService.enabled = true;
        (setup.services.errorMessagesService as ErrorMessagesService).registerErrorMessage(ConditionType.RegExp, null, {
            '*': 'Error Found'
        });
        let onValStateChanged: ValueHostValidationState | null = null;
        setup.valueHostsManager.onValueHostValidationStateChanged = (valueHost, validateResult) => {
    ...
    test('Register a DataTypeCheckCondition for PhoneNumber and ensure it gets autogenerated and used', () => {
        ...
        logger.minLevel = LoggingLevel.Info;
        services.dataTypeCheckGeneratorService.enabled = true;
        (services.dataTypeCheckGeneratorService as DataTypeCheckGeneratorService).
            register(new PhoneNumberDataTypeCheckGenerator());

        (services.errorMessagesService as ErrorMessagesService).
        ...
        vhm.onValueHostValidationStateChanged = (valueHost, validateResult) => {
                onValStateChanged = validateResult;
            }
    ...
    // test built-in registered generators
    // IntegerDataTypeCheckGenerator by using LookupKey.Integer
    test('IntegerDataTypeCheckGenerator is registered and supports LookupKey.Integer', () => {
        ...
        logger.minLevel = LoggingLevel.Info;
        setup.services.dataTypeCheckGeneratorService.enabled = true;
        setup.services.dataTypeCheckGeneratorService.register(new IntegerDataTypeCheckGenerator());
        expect(setup.services.dataTypeCheckGeneratorService.find(LookupKey.Integer)).toBeInstanceOf(IntegerDataTypeCheckGenerator);
        
        (setup.services.errorMessagesService as ErrorMessagesService).registerErrorMessage(
            ConditionType.Integer, LookupKey.Integer, {
            '*': 'Error Found as Integer'
        });
        ...
        // Using decimals instead of strings because string into LookupKey integer
        // violates the DataTypeCheckCondition which is also added.
        // IntegerDataTypeCheckGenerator takes on the case where we have a number, but not an integer
        setup.valueHost.setValues(1.5, '1.5');   // will violate IntegerDataTypeCheckGenerator
        setup.valueHost.validate();
        expect(onValStateChanged).toEqual(
            <ValueHostValidationState> {
                issuesFound: [{
                    errorCode: ConditionType.Integer,
                    valueHostName: 'Field1',
                    severity: ValidationSeverity.Severe,    // due to DataTypeCheck
                    errorMessage: 'Error Found as Integer',
                    summaryMessage: 'Error Found as Integer',
                    doNotSave: true
                }],
                isValid: false,
                doNotSave: true,
    ...
    // use registerLookupKey('email', regexp for email)
    test('registerLookupKey for email works correctly', () =>
    {
        const lookupKey = 'Email';
        const regexp = /^([\w\.!#\$%\-+.'_]+@[A-Za-z0-9\-]+(\.[A-Za-z0-9\-]{2,})+)/i;
        let setup = setupFieldValueHost({
            name: 'Field1',
            dataType: lookupKey
        });
        ...
        logger.minLevel = LoggingLevel.Info;
        setup.services.dataTypeCheckGeneratorService.enabled = true;
        setup.services.dataTypeCheckGeneratorService.registerLookupKey(lookupKey, regexp);
        expect(setup.services.dataTypeCheckGeneratorService.find(lookupKey)).toBeInstanceOf(RegExpDataTypeCheckGenerator);

        (setup.services.errorMessagesService as ErrorMessagesService).registerErrorMessage(
            ConditionType.RegExp, lookupKey, {
            '*': 'Error Found as Email'
        });
        ...
        setup.valueHost.setValues('ABC', 'ABC');   // will violate RegExpDataTypeCheckGenerator
        setup.valueHost.validate();
        expect(onValStateChanged).toEqual(
            <ValueHostValidationState> {
                issuesFound: [{
                    ...
                    valueHostName: 'Field1',
                    severity: ValidationSeverity.Severe,    // due to DataTypeCheck
                    errorMessage: 'Error Found as Email',
                    summaryMessage: 'Error Found as Email',
                    doNotSave: true
                }],
                isValid: false,
                doNotSave: true,
...
describe('getElementIdentifier and setElementIdentifier', () =>
{
    ...
    test('setElementIdentifier changes the elementIdentifier and getElementIdentifier returns the new value', () =>
    {
        ...
        setup.valueHostsManager.onNotifyValueHostInstanceStateChanged = (valueHost: IValueHost, instanceState: ValueHostInstanceState): void =>
        {
...
describe('No mocks!', () => {
    function createServices(logLevel: LoggingLevel = LoggingLevel.Debug): IJivsServices {
        ...
        services.loggingService.minLevel = logLevel;
        return services;
    }
    ...
    test('setValue with validate=true, onValueHostValidationStateChanged called', () => {
        ...
        let logger = services.loggingService as ConsoleLoggingService;
