...
;
function setupInputValueHost(descriptor, state) {
    ...
    let updatedState = (!state) ?
        defaultState : Object.assign(Object.assign({}, defaultState), state);
    let vh = new BusinessLogicInputValueHost(vm, updatedDescriptor, updatedState);
    return {
        services: services,
...
describe('BusinessLogicInputValueHost.validate', () => {
    ...
    test('One BusinessLogicErrors with only ErrorMesage results in ValidationResult.Invalid and one IssueFound because Severity=undefined means Severity=Error', () => {
        ...
        expect(vr.issuesFound[0]).toEqual({
    ...
    test('2 BusinessLogicErrors (Warning, Error) results in ValidationResult.Invalid and two IssueFounds', () => {
        ...
        expect(vr.issuesFound[1]).toEqual({
...
//# sourceMappingURL=BusinessLogicInputValueHost.test.js.map